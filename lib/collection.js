...
Collection.prototype.createTable = function(cb) {
  ...
  async.series(
    [
      // create table
      function(cb) {
        self.client.execute(self.ddlHelper.cqlCreateTable, function(err) {
      ...
      function(cb) {
        async.eachSeries(_.values(self.ddlHelper.indexes),
          function(item, cb) {
            self.client.execute(item, function(err) {
              if (err) return cb(err);
              cb();
            });
          },
...
Collection.prototype.describeTable = function(cb) {
  ...
  var minVersion = null;
  this.client.hosts.forEach(function(host) {
    var hostVer = host.cassandraVersion;
    if(hostVer.split('.').length < 3){
      hostVer+='.0';
    }
    if (!minVersion || semver.lt(hostVer, minVersion)) {
      minVersion = host.cassandraVersion;
    }
  });

  // this is for cassandra 3.x
  var cql = "SELECT * FROM system_schema.columns WHERE keyspace_name='" + this.client.keyspace + "'";
  cql +=  " AND table_name='" + this.ddlHelper.tableName + "';";

  // cassandra 2.x has different layout of system tables
  if (semver.satisfies(minVersion, '^2.0.0')) {
    cql = "SELECT * FROM system.schema_columns WHERE keyspace_name='" + this.client.keyspace + "'";
    cql +=  " AND columnfamily_name='" + this.ddlHelper.tableName + "';";
  }

  this.client.eachRow(cql, null, null,
    (n, row) => {
      var attrName = this.ddlHelper.colNamesToAttrNames[row.column_name];
      if (attrName) {
        collection[attrName] = {};
    ...
    (err, result) => {
      ...
      if (result.rowLength > 0) {
        ...
        return cb(null, _.merge({}, this.schema, collection));
      }
      cb();
    }
  );
...
Collection.prototype.insert = function(items, cb) {
  ...
  items.forEach(function(item) {
    ...
    if(this.ddlHelper.pkAttrNames.length > 1){
      if(this.ddlHelper.pkAutoIncrement){
        return cb(new Error("Autoincrement feature is not available to table with composite keys."));
      }

      this.ddlHelper.pkAttrNames.forEach(function(pkAttrName){
        if (!(pkAttrName in item)) {
          return cb(new Error("missing primary key '"+ pkAttrName + "'"));
        }
      });
    }else{
      var pkAttrName = this.ddlHelper.pkAttrNames[0];
      if (pkAttrName in item) {
        if (this.ddlHelper.pkAutoIncrement) {
        ...
        if (this.ddlHelper.pkAutoIncrement) {
          item[pkAttrName] = new TimeUuid().toString();
        }
        else {
...
Collection.prototype.update = function(criteria, values, cb) {
  ...
  var pkColNames = [];
  self.ddlHelper.pkAttrNames.forEach(function(pkAttrName){
    pkColNames.push(self.ddlHelper.attrNamesToColNames[pkAttrName]);
  });

  var cqlSelect = "SELECT " + pkColNames.join(',') + " FROM " + self.ddlHelper.tableName + " WHERE";
  ...
  cqlUpdate = cqlUpdate.replace(/,$/, ' WHERE "' + pkColNames.join('" = ? AND "') + '" = ?;');
  ...
  self.ddlHelper.pkAttrNames.forEach(function(pkAttrName){
    if (self.ddlHelper.attrNamesToColNames[pkAttrName] in where) {
      if(!idVals || !idCols){
        idVals = [];
        idCols = [];
      }
      var key = self.ddlHelper.attrNamesToColNames[pkAttrName];
      idCols.push(key);
      idVals.push(where[key]);
    }else if(pkAttrName in where){
      if(!idVals || !idCols){
        idVals = [];
        idCols = [];
      }
      idCols.push(pkAttrName);
      idVals.push(where[pkAttrName]);
    }
  });
  ...
  if (idCols && idVals) {
    ...
    // build update CQL
    cqlUpdateParams = cqlUpdateParams.concat(idVals);
    updates = [ values ];
    for(var i = 0;i < idCols.length;i++){
      updates[0][idCols[i]] = idVals[i];
    }
...
Collection.prototype._parseDefinition = function(definition) {
  ...
  Object.keys(this.schema).forEach(function(attrName) {
    ...
    if (attribute.primaryKey) {
      ...
      if(!this.ddlHelper.pkAttrNames){
        this.ddlHelper.pkAttrNames = [];
      }
      this.ddlHelper.pkAttrNames.push(attrName);
      // if (this.ddlHelper.pkAttrName) {
    ...
    // when index == true, use regular index
    if (attribute.index === true) {
      indexColNames.push(colName);
    }else if(attribute.index && (attribute.index.toUpperCase() == 'DESC' || attribute.index.toUpperCase() == 'ASC')){
      clustringColNames[colName] = attribute.index.toUpperCase();
    }
  ...
  var pkColNames = [];
  for(var index in this.ddlHelper.pkAttrNames){
    var pkAttName = this.ddlHelper.pkAttrNames[index];
    pkColNames.push(this.ddlHelper.attrNamesToColNames[pkAttName]);
  }

  var ccql = '';
  if(Object.keys(clustringColNames).length){
    clusteringCQL = ', "' + Object.keys(clustringColNames).join('", "') + '"';
    cql += 'PRIMARY KEY (("' + pkColNames.join('","') + '") '+ clusteringCQL + ')) ';

    var clustringColNamesWithOrder = [];
    Object.keys(clustringColNames).forEach(function(key){
      clustringColNamesWithOrder.push('"' + key + '" ' + clustringColNames[key]);
    });

    ccql += 'WITH CLUSTERING ORDER BY (' + clustringColNamesWithOrder.join(', ') + ')';


  }else{
    cql += 'PRIMARY KEY ("' + pkColNames.join('","') + '"));' + ccql;

  }

  this.ddlHelper.cqlCreateTable = cql+ccql;
...
Collection.prototype._parseCriteria = function(value, key, parentKey) {
  ...
  if (!key) {
    ...
    // in pair
      ...
      var questionMarks = '?';
      for(var count = 1; count < value.length; count ++){
       questionMarks = '?, ' + questionMarks;
      }
      cql = '"' + key + '" IN (' + questionMarks + ')';
...
Collection.prototype._constructSelectQuery = function(criteria) {
  ...
  var fields = Object.keys(this.ddlHelper.colTypes);
  var select = criteria.select || criteria.fields;

  // filter by custom select or fields option
  if(select){
    for(var i in fields){
      if(select.indexOf(fields[i]) == -1){
        delete fields[i];
      }
    }
  }
  ...
  if (where) {
    ...
    //   cql += ' WHERE ';
    //   cqlParams = [];
    //   for(var index in idCols){
    //     idCol = idCols[index];
    //     idVal = idVals[index];
    //     if(typeof idVal == 'string'){
    //       cql += '"' + idCol + '" = ?';
    //       cqlParams.push(idVal);
    //     }else{
    //       console.log("====== IDVAL:");
    //       console.log(idVal);
    //       var questionMarks = '?';
    //       cqlParams.push(idVal[0]);
    //       for(var count = 1; count < idVal.length; count ++){
    //         questionMarks = '?, ' + questionMarks;
    //         cqlParams.push(idVal[count]);
    //       }
    //
    //       cql += '"' + idCol + '" in (' + questionMarks +' )';
    //     }
    //     if(index != idCols.length - 1 ){
    //       cql += ' AND ';
    //     }
    //   }
    // }
